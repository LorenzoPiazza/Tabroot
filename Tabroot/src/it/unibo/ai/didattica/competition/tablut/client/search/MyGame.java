...
import it.unibo.ai.didattica.competition.tablut.client.search.*;
...
public class MyGame extends GameAshtonTablut implements Game<State, Action, Turn> {
	...
	private double getSpecificHeuristicValueWhite(State state, List<int[]> white, List<int[]> black, int[] king) {
		...
		/*Guardo l'assetto torre (Per torre si intende che nella scacchiera ho 4 pedine, di cui una � il re,
		...
		*/
		...
		return 0.53*conteggioPedine + 0.05*scappaRe + 0.05*pedineInAngoli + 0.04*vicinanzaBordiAngoli + 0.02*assettoTorre+0.20*mosseIntelligenti+0.25*valutaQuadrantiKing+0.001*(new Random().nextDouble());
...
//		return 0.53*conteggioPedine + 0.7*scappaRe + 0.10*pedineInAngoli + 0.20*mosseIntelligenti+0.22*valutaQuadrantiKing;
	...
	private double getGeneralHeuristicValueWhite(State state, List<int[]> white, List<int[]> black, int[] king) {
		...
		*/
		...
		return 0.53*conteggioPedine + 0.05*scappaRe + 0.05*pedineInAngoli + 0.04*vicinanzaBordiAngoli + 0.02*assettoTorre+0.20*mosseIntelligenti+0.25*valutaQuadrantiKing;
	...
	private double getSpecificHeuristicValueBlack(State state, List<int[]> white, List<int[]> black, int[] king) {
		...
		return 0.50*conteggioPedine+0.34*assettoGabbiaLight+0.15*assettoGabbiaStrong+0.001*(new Random().nextDouble());
	}
